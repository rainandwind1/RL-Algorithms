...
class RND(nn.Module):
    ...
    def cal_rewards(self, s, a):
        a = torch.tensor(a, dtype = torch.float32)
        inputs = torch.cat((s, a), 1)
        fixed_op = self.fixed_net(inputs)
        target_op = self.target_net(inputs)
        loss = torch.nn.functional.mse_loss(fixed_op, target_op)
        # rnd_reward = loss.detach().numpy()  # 单体计算
        rnd_reward = loss.detach()
...
def train(model, rnd, loss_fn, loss_list, score_list):
    s, a, r, s_next, a_prob, done_flag = model.package_trans()
    r += rnd.cal_rewards(s, a)
        ...
        while not done:
            for step in range(T_HORIZON):
                ...
                if done:
                    ...
                    score = 0.
                    break
            train(model, rnd, loss_fn=None, loss_list=None, score_list=None)
    env.close()